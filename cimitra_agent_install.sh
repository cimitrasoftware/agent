...
echo "AGENT_INSTALL_PROCESS_SUCCESS = $AGENT_INSTALL_PROCESS_SUCCESS"
...
then
	...
	if [ $OS_IS_MAC -eq 0 ]
	then
	${SUDO_COMMAND} cimitra import -b -u ${CIMITRA_SERVER_ADMIN_ACCOUNT} -p ${CIMITRA_SERVER_ADMIN_PASSWORD} name="[ AGENT: ${CIMITRA_AGENT_IN} ] HOST INFO"^ command=echo '[ Command: uname -n ]' ";" uname -n ";" echo ";" echo '[ Command: uname -a ]' ";" uname -a ";" echo ";" echo '[ Command: ip address show ]' ";" ip address show ";" echo '[ Command: lscpu ]' ";" echo ";" lscpu ";" echo '[ Command: lsblk ]' ";" echo ";" lsblk ";" echo '[ Command: df -hl ]' ";" echo ";" df -hl ";" echo '[ Command: top -bn1 ]' ";" echo ";" top -bn1^
	else
	sudo cimitra import -b -u ${CIMITRA_SERVER_ADMIN_ACCOUNT} -p ${CIMITRA_SERVER_ADMIN_PASSWORD} name="[ AGENT: ${CIMITRA_AGENT_IN} ] HOST INFO"^ command=echo '[ Command: uname -n ]' ";" uname -n ";" echo ";" echo '[ Command: uname -a ]' ";" uname -a ";" echo ";" echo '[ Command: sysctl -n machdep.cpu.brand_string ]' ";" sysctl -n machdep.cpu.brand_string ";" echo ";" echo '[ Command: ifconfig ]' ";" ifconfig ";" echo '[ Command: sw_vers ]' ";" echo ";" sw_vers ";"  echo '[ Command: df -hl ]' ";" echo ";" df -hl^
	fi
	
	${SUDO_COMMAND} cimitra import -b -u ${CIMITRA_SERVER_ADMIN_ACCOUNT} -p ${CIMITRA_SERVER_ADMIN_PASSWORD} name="[ AGENT: ${CIMITRA_AGENT_IN} ] PING"^ command=echo ";" ping ^  inputone=-c 2@ADDRESS@ADDRESS@false^
