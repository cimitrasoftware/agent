#!/bin/bash
###########################################
# cimitra_nix_agent_install.sh            #
# Author: Tay Kratzer - tay@cimitra.com   #
# Version: 1.0                            #
# Modify date: 6/4/2020                   #
###########################################
...
declare -i INPUT_LENGTH=`echo "${PASS_INPUT}" | wc -m`
...
if [ $CIMITRA_AGENT_NOT_INSTALLED -eq 1 ]
then
...
CIMITRA_BINARY_AGENT_ID=`sudo node ${CIMITRA_AGENT_BINARY_FILE} | grep -iA1 "agentid" | tail -1 | awk -F "= " '{printf $2}'`
	fi
fi
if [ $DEBUG -eq 1 ]
then
...
declare -i CIMITRA_STATUS=`echo $?`
	...
	then
		...
		CALL_COMMAND "sudo node ./cimagent.js c"
		sudo node ./cimagent.js c		
		fi
	fi
	...
	then
		...
		then
			...
			CALL_COMMAND "sudo node ./cimagent.js c systemd"
			sudo node ./cimagent.js c systemd		
			fi
...
then
	...
	if [ $OS_IS_MAC -eq 0 ]
	then
	sudo cimitra import -b -u ${CIMITRA_SERVER_ADMIN_ACCOUNT} -p ${CIMITRA_SERVER_ADMIN_PASSWORD} name="[ AGENT: ${CIMITRA_AGENT_IN} ] HOST INFO"^ command=echo '[ Command: hostname ]' ";" hostname ";" echo ";" echo '[ Command: uname -a ]' ";" uname -a ";" echo ";" echo '[ Command: ip address show ]' ";" ip address show ";" echo '[ Command: lscpu ]' ";" echo ";" lscpu ";" echo '[ Command: lsblk ]' ";" echo ";" lsblk ";" echo '[ Command: df -hl ]' ";" echo ";" df -hl ";" echo '[ Command: top -bn1 ]' ";" echo ";" top -bn1^
	else
	sudo cimitra import -b -u ${CIMITRA_SERVER_ADMIN_ACCOUNT} -p ${CIMITRA_SERVER_ADMIN_PASSWORD} name="[ AGENT: ${CIMITRA_AGENT_IN} ] HOST INFO"^ command=echo '[ Command: hostname ]' ";" hostname ";" echo ";" echo '[ Command: uname -a ]' ";" uname -a ";" echo ";" echo '[ Command: sysctl -n machdep.cpu.brand_string ]' ";" sysctl -n machdep.cpu.brand_string ";" echo ";" echo '[ Command: ifconfig ]' ";" ifconfig ";" echo '[ Command: sw_vers ]' ";" echo ";" sw_vers ";"  echo '[ Command: df -hl ]' ";" echo ";" df -hl^
	fi
	
	sudo cimitra import -b -u ${CIMITRA_SERVER_ADMIN_ACCOUNT} -p ${CIMITRA_SERVER_ADMIN_PASSWORD} name="[ AGENT: ${CIMITRA_AGENT_IN} ] PING"^ command=echo ";" ping ^  inputone=-c 2@ADDRESS@ADDRESS@false^
