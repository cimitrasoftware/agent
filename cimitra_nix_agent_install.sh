...
PROGRAM_NAME="cimitra_nix_agent_install.sh"
...
CALL_INFO "[ Cimitra Linux/MacOS Agent Install Script - Begin ]"
...
declare -i OS_IS_LINUX=`echo "${OSTYPE}" | grep -c "linux"`

declare -i OS_IS_MAC=`echo "${OSTYPE}" | grep -c "linux"`
...
CIMITRA_AGENT_NOT_INSTALLED=`test -f ${CIMITRA_AGENT_BINARY_FILE} ; echo $?`
...
declare -i CIMITRA_AGENT_NAME_EXISTS=`cat ${TEMP_FILE_TWO} | grep -icw "name:${AGENT_NAME}"`
	...
	CIMITRA_PAIRED_AGENT_ID=`cat ${TEMP_FILE_TWO} | grep -iwB 1 "name:${AGENT_NAME}" | head -1 | awk -F ":" '{printf $2}'`
		...
		if [ $DEBUG -eq 1 ]
		then
		CALL_INFO "An Agent By the Name of ${AGENT_NAME} Exists"
		fi
...
CALL_INFO "[ Cimitra Linux/MacOS Agent Install Script - End ]"
