...
Write-Host ""
Write-Host ""
Write-Host "Reinstall Agent Binary (.EXE) File  (Prompt for credentials method)"
Write-Host ""
Write-Host ".\$scriptName <cimitra server address> <cimitra server port> reinstall"
Write-Host ""
Write-Host "Example:"
Write-Host ""
Write-Host ".\$scriptName 192.168.1.10 443 reinstall"
Write-Host ""
Write-Host "Script Help"
Write-Host ""
...
$agentNameLine = (Get-Content $TEMP_FILE_ONE) -replace " ", "" | Select-String -Pattern "name:"

$agentName = Write-Output "$agentNameLine" | %{ $_.Split(':')[1]; }
...
$IMAGE_PATH = Get-Content $TEMP_FILE_ONE | Select-String -Pattern "cimitra.cfg" 

$IMAGE_PATH = $IMAGE_PATH -replace ' ', ''

Remove-Item -Path $TEMP_FILE_ONE -Force

$CIMITRA_AGENT_CONFIG_FILE = $IMAGE_PATH -replace '"', ""

$global:CIMITRA_AGENT_CONFIG_FILE_PATH = "${CIMITRA_AGENT_CONFIG_FILE}"

Write-Output "CONFIG = ${CIMITRA_AGENT_CONFIG_FILE}"
...
# Write-Output "Cimitra Agent Unique ID = $CIMITRA_AGENT_UNIQUE_ID"
CHECK_CONNECTIVITY
...
$AGENT_LINE = Select-String -Path $DEFINED_AGENT_DUMP -CaseSensitive -Pattern "\b:_id\b" 1> $DEFINED_AGENT_ID_LINE_ONE
...
$AGENT_ID_ONE = (Get-Content $DEFINED_AGENT_ID_LINE_TWO) | %{ $_.Split('_id:')[1]; } 


$AGENT_ID = ("$AGENT_ID_ONE".Trim())


if( "$AGENT_ID".Length -ne 24 ){
Write-Output ""
...
$CIMITRA_DOWNLOAD = "https://github.com/cimitrasoftware/agent/blob/master/cimitra.ps1"
