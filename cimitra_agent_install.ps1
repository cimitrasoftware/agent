...
# Invoke-RestMethod -Uri $uri/users/login -Method POST -Body $payload -ContentType "application/json"
...
$response = Invoke-RestMethod -Uri $uri/users/login -Method POST -Body $payload -ContentType "application/json"
    ...
    try{
    $response = Invoke-WebRequest -Uri $uri/users/login -Method POST -Body $payload -ContentType "application/json"
    }catch{
...
try{
Get-Service Cimitra 2>&1 | out-null
}catch{
...
if(!($legacyPowershell)){
    ...
    try{
    $RESPONSE = Invoke-RestMethod -SkipCertificateCheck -Uri $uri/agent/$cimitraAgentId -Method GET -Headers $headers -UseBasicParsing  -ContentType "application/json"
    }catch{
    ...
    try{
    $RESPONSE = Invoke-RestMethod -Uri $uri/agent/$cimitraAgentId -Method GET -Headers $headers -UseBasicParsing  -ContentType "application/json"
    }catch{
    Write-Output "The Locally Defined Cimitra Agent Does Not Exist on The Cimitra Server Specified" 
    return
}
...
try{
if(!($legacyPowershell)){
Invoke-RestMethod -SkipCertificateCheck -Uri "${URL}" `
-Method "POST" `
...
try{
Invoke-WebRequest -Uri $uri/agent -Method GET -Headers $headers -UseBasicParsing > $ALL_AGENTS_DUMP
}catch{

    try{
    if(!($legacyPowershell)){
    Invoke-RestMethod -SkipCertificateCheck -Uri $uri/agent -Method GET -Headers $headers -UseBasicParsing > $ALL_AGENTS_DUMP
    }else{
...
try{
    if(!($legacyPowershell)){
    Invoke-RestMethod -SkipCertificateCheck -Uri $uri/agent/${cimitraAgentId} -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
    }else{
...
$rootFolderIdOne > $TEMP_FILE_ONE

$ID_LINE = Select-String -Path $TEMP_FILE_ONE -Pattern "_id:"

Remove-Item -Path $TEMP_FILE_TWO -Force 2>&1 | out-null

$rootFolderId = ($ID_LINE).ToString().Split(":")[-1]
...
try{
Get-Service -Name $ServiceName 2>&1 | out-null
}catch{return}
...
try{
Start-BitsTransfer -Source $CIMITRA_DOWNLOAD -Destination $CIMITRA_DOWNLOAD_OUT_FILE
# Invoke-RestMethod $CIMITRA_DOWNLOAD -OutFile $CIMITRA_DOWNLOAD_OUT_FILE -UseBasicParsing 2>&1 | out-null
}catch{
